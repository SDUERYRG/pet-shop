            ...
            <div class="maindown">
                ...
                <div class="orderItem" v-for="order in orderItems" :key="order.orderId">
                    ...
                    <div style="display: flex;flex-direction: row;width: 100%;">
                        <div style="width: 47%;">
                        <div class="imgname" v-for="(itemName, index) in order.orderDetail?.itemName" :key="order.orderDetail?.itemId[index]">
                            <div class="img">
                                <img :src="showImg(order.orderDetail!.itemId[index]) ?? 'default-image-url.jpg'" :alt="showImg(order.orderDetail!.itemId[index]) ?? ''" style="width: 100px;height: 100px;">
                            </div>
                        ...
                        <div style="width: 10%;display: flex;flex-direction: column;justify-content: center;">
                            <ElButton type="text">确认收货</ElButton>
                        </div>
                    </div>
...
setup() {
  ...
  const logout = async () => {
    ...
    const getOrder = async () => {
        ...
        const response = await axios.get(`http://localhost:4001/diary-server/order/userOrder/${userId}/0/10`, {
            headers: {
                'Content-Type': 'application/json',
                'token': localStorage.getItem('token'),
            },
